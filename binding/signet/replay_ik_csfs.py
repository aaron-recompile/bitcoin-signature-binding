"""
Signet — IK+CSFS replay: two funds to the same internal key + cbcc leaf, two spends with the same [sig, message].

  PYTHONPATH=. python -m binding.signet.replay_ik_csfs --fund-twice
  PYTHONPATH=. python -m binding.signet.replay_ik_csfs --spend [--input-txid TXID]

MESSAGE = SHA256(b\"binding-exp-msg:ik_csfs\") matches offline ik_csfs_case.py.
"""
...
STATE_FILE = "state_replay_ik_csfs.json"
FUND_HINT_FILE = str(SIGNET_DIR / ".replay_ik_csfs_last_fund_txid")


def _make_witness(wif: str) -> list[str]:
    secret = wif_secret_bytes(wif)
    sk = PrivateKey(secret, raw=True)
    sig = sk.schnorr_sign(MESSAGE_IK_CSFS, None, raw=True)
    return [sig.hex(), MESSAGE_IK_CSFS.hex()]


def _program(wif: str):
    key = Key.from_wif(wif)
    leaf = RawScript(build_script(OP_INTERNALKEY, OP_CHECKSIGFROMSTACK))
    return (
        TapTree(internal_key=key, network="signet").custom(script=leaf, label="ik_csfs").build()
    )
...
def cmd_fund_twice(wif: str) -> None:
    ...
    st.update(
        {
            "demo": "replay_ik_csfs",
            "address": addr,
            "message_label": "binding-exp-msg:ik_csfs",
            "fund_txids": [t1, t2],
        }
    )
    ...
    print("Same [sig, message] witness unlocks both UTXOs (same internal key).")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="IK+CSFS Signet replay (two funds, same witness)")
    parser.add_argument("--fund-twice", action="store_true")
    parser.add_argument("--spend", action="store_true")
    parser.add_argument("--input-txid", metavar="TXID")
